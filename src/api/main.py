...
class HousingFeatures(BaseModel):
    ...
    AveRooms: float = Field(
        ..., ge=0, description="Average number of rooms per household"
    )
    AveBedrms: float = Field(
        ..., ge=0, description="Average number of bedrooms per household"
    )
    Population: float = Field(..., ge=0, description="Block group population")
    AveOccup: float = Field(
        ..., ge=0, description="Average number of household members"
    )
...
async def predict_price(features: HousingFeatures):
    ...
    with prediction_duration.time():
        try:
            ...
            prediction_logger.log_prediction(features.dict(), prediction, response_time)
